...
function tile(items: WidgetItem[], columns: number): Placement[] | null {
	...
	const grid: boolean[] = Array.from({ length: rows * columns }, () => false)
	const used: boolean[] = Array.from({ length: items.length }, () => false)
	const out: Placement[] = []
...
function pack(items: WidgetItem[], columns: number): Placement[] {
	...
	while (queue.length > 0) {
		...
		const cells: boolean[] = Array.from({ length: height * columns }, () => false)
